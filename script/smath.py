...
def calcSinCos(period, clock, ncycle, data):
	...
	#print "ncycle=", ncycle
	fsin = [ math.sin(2*math.pi*i/ncycle) for i in range(ncycle)]
	fcos = [ math.cos(2*math.pi*i/ncycle) for i in range(ncycle)]
...
def deltaError(data, c0, amplitude, fi, ncycle):
	N = len(data)
	out = [ c0+amplitude*math.sin(2*math.pi*i/ncycle+fi)-data[i] for i in range(N)]
	return out
